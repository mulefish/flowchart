...
function save() {
    ...
    const x = JSON.stringify(graphData);
    localStorage.setItem("graph", x)
    const rightNow = new Date()
    document.getElementById("graphJson").value = "save " + rightNow
}


function load() {
    const x = localStorage.getItem("graph")
    graph = JSON.parse(x);
    
    const rightNow = new Date()
    document.getElementById("graphJson").value = "load " + rightNow
}


function getPiniaReadyObject() {
    const nodes = [];
    graph.forEach((shape) => {
        nodes.push({
            letter: shape.letter,
