...
function populateSignup(logArea) {
  try {
    ...
    selectOption("Jr", {
      ...
      if (cb) {
          const checkboxWrapper = cb.closest('.q-checkbox'); // Find the wrapper
...
function populateBirthAndCitizenship() {
  try {
    ...
    const cityInput = document.querySelector('input[aria-testid="webcountrycitystateselector-city-q-input"]');
    if (cityInput) {
      cityInput.focus();
