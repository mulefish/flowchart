...
selectFromQSelect('countrycitystateselector-state-q-select', 'Alabama');
const cityInput = document.querySelector(
  'input[aria-testid="webcountrycitystateselector-city-q-input"]'
);

if (!cityInput) {
     logArea.value += 'City input not found'
} else {
  // focus/open if needed
  cityInput.focus();

  // set the value
  cityInput.value = 'version3';

}


            logArea += "Try to next btn!"
    ...
    function getPopulateConfig(pathname) {
      switch (true) {
        ...
        case pathname.endsWith("/signup") || pathname === "/":
          ...
          case pathname.endsWith("/birthAndCitizenship"):
              return { handler: populateBirthAndCitizenship, btnText:"opulate BirthAndCitizenship"};
          default:
